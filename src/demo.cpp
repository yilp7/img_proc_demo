...
bool Demo::load_image_tif(cv::Mat &img, QString filename)
{
...
bool Demo::load_image_file(QString filename, bool init)
{
    ...
    if (tiff) {
        if (Demo::load_image_tif(mat_temp, filename)) return false;
    }
    else {
...
void Demo::start_static_display(int width, int height, bool is_color, int pixel_depth)
{
    if (grab_thread_state || h_grab_thread) {
        grab_thread_state = false;
        if (h_grab_thread) {
